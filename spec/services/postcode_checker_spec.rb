...
RSpec.describe PostcodeChecker, type: :model do
  ...
  let(:valid_postcode) { described_class.new('SE1 7QD') }
  let(:out_of_area) { described_class.new('IP13 0SR') }
  let(:invalid_postcode) { described_class.new('invalid') }
  let(:allowed_postcode) { described_class.new(valid_postcode_allow_list.postcode) }
  ...
  context 'valid postcode data' do
    ...
    it 'should return success for allow-listed postcode' do
      ...
      pc = PostcodeChecker.new(postcode)
      response = pc.check
      expect(response).to eq({ status: 'success', message: '', in_area: true })
    end
  ...
  context 'unable to reach external api' do
    it 'should return failure' do
      response = valid_postcode.check
      expect(response).to eq({ status: 'failure', message: 'Unable to check Postcode', in_area: nil })
    end
  end
  ...
  context 'invalid postcode data' do
    it 'should return failure for invalid postcode' do
      VCR.use_cassette('postcodes_io_invalid_postcode') do
        response = valid_postcode.check
        expect(response).to eq({ status: 'failure', message: PostcodesIoClient::INVALID_POSTCODE_FORMAT_MSG, in_area: nil })
      end
    end
  end
end
